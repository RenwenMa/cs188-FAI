...
def fillYCPT(bayesNet, gameState):
    ...
    "*** YOUR CODE HERE ***"
    for val,prob in [(BOTH_TOP_VAL, PROB_BOTH_TOP), (BOTH_BOTTOM_VAL, PROB_BOTH_BOTTOM),
     (LEFT_TOP_VAL, PROB_ONLY_LEFT_TOP), (LEFT_BOTTOM_VAL, PROB_ONLY_LEFT_BOTTOM)]:
...
def fillObsCPT(bayesNet, gameState):
    ...
    posToVar = {bottomLeftPos : BOTTOM_LEFT_VAL, bottomRightPos : BOTTOM_RIGHT_VAL,
    topLeftPos : TOP_LEFT_VAL, topRightPos : TOP_RIGHT_VAL}
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in  gameState.getHouseWalls(housePos):
            ...
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                if assignment[obsVar] == NO_OBS_VAL and \
                posToVar[housePos] not in [assignment[FOOD_HOUSE_VAR], assignment[GHOST_HOUSE_VAR]]:
                    obsFactor.setProbability(assignment, 1)

                elif posToVar[housePos] == assignment[FOOD_HOUSE_VAR]:
                ...
                elif posToVar[housePos] == assignment[GHOST_HOUSE_VAR]:
                    if assignment[obsVar] == RED_OBS_VAL:
