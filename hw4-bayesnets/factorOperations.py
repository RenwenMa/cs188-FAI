...
def normalize(factor):
    ...
    filtered = filter(lambda var: len(variableDomainsDict[var]) == 1, unconditioned)
    conditioned.update(filtered)
    unconditioned.difference_update(filtered)

    normalized_factor = Factor(unconditioned, 
        conditioned, variableDomainsDict)
    sum_prob = sum(map(lambda asg: factor.getProbability(asg), factor.getAllPossibleAssignmentDicts()))

    for assignment in normalized_factor.getAllPossibleAssignmentDicts():
        normalized_factor.setProbability(assignment, factor.getProbability(assignment)/sum_prob)
